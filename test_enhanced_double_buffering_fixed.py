...
def main():
    ...
    # 创建临时输出目录
    temp_output_dir = tempfile.mkdtemp(prefix="mineru_fixed_test_")
    ...
    try:
        ...
        # 等待预处理完成，但不立即发送完成信号
        print("\n等待预处理完成...")
        max_preprocess_wait = 60  # 最大等待60秒预处理
        preprocess_start_time = time.time()

        while time.time() - preprocess_start_time < max_preprocess_wait:
            elapsed = time.time() - preprocess_start_time
            preprocess_size = pool.get_preprocessing_queue_size()
            gpu_size = pool.get_gpu_queue_size()

            print(f"等待预处理中...已等待{elapsed:.1f}秒，队列状态 - 预处理: {preprocess_size}, GPU: {gpu_size}")

            # 如果预处理队列为空且GPU队列有任务，说明预处理完成
            if preprocess_size == 0 and gpu_size > 0:
                print("检测到预处理完成，任务已提交到GPU队列")
                break
        ...
        while time.time() - start_time < max_wait_time:
            ...
            # 尝试获取结果
            try:
                result = pool.get_result(timeout=10.0)  # 10秒超时
                if result:
                    print(f"  收到结果!")
                    break
                else:
